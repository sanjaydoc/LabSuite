...
def test_seeded_trained_user_has_gated_access():
    cp = build_lab()
    report = cp.resolve_access("anguyen")  # established research scientist, trained
...
def test_fresh_hire_is_gated_until_trained():
    ...
    # Required trainings start pending, so the gated shares are withheld.
    ...
    # ...but non-gated access is granted immediately.
...
def test_completing_training_unlocks_access():
    cp = build_lab()
    cp.onboard("Nadia Rahman", Department.RESEARCH, "research-scientist")
...
def test_lapsed_training_auto_revokes():
    cp = build_lab()
    cp.expire_training("anguyen", "Biosafety")  # established user's training lapses
    report = cp.resolve_access("anguyen")
    assert "invivo-study-data" in report.blocked
    assert "invivo-study-data" not in report.truenas
    assert not cp.check_truenas("anguyen", "invivo-study-data", "modify").allowed


def test_review_flags_compliance_issues():
    cp = build_lab()
    cp.expire_training("anguyen", "Biosafety")
